...
test("ships a loader-safe Sacred Valley terrain GLB", () => {
  ...
  // Draco is required and that is fine: the decoder ships in `public/draco/`.
  expect(manifest.extensionsRequired).toEqual(["KHR_draco_mesh_compression"]);
  expect(isHeroGltfLoaderSafe(manifest)).toBe(true);
  // The drape is a plain JPEG on purpose: WebP and Basis both need loader
  // extensions that `isHeroGltfLoaderSafe` refuses.
...
test("reads the terrain subject from GLB extras", () => {
  ...
  expect(readHeroSubject({ heroSubject: "skyline" }, undefined)).toBe(null);
  expect(
    heroSubjectFromLoadedGltf({
